...
const dateNumber = decodeURIComponent(urlParams.get("dateNumber"));
...
const categoryCode = decodeURIComponent(urlParams.get("categoryCode"));
const quantity = decodeURIComponent(urlParams.get("quantity"));
...
let initQuantity = "", initPrice = "", initDesc = ""; //초기값.
if(mode === "modify") {
    // 수정 모드
    // 값 설정
    if (date.length === 3) {
    ...
    function save() {
        const result = DataStore.getInstance().updateSale(dateNumber, categoryCode, quantity, price, description);
        if(result === true) {
            alert("수정이 완료되었습니다.");
            if(window.opener) {
