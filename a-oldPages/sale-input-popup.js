...
if (mode === "modify") {
    ...
    // }
    const dateNumberParsed = dateNumber.split("-");
    const year = dateNumberParsed[0];
    const month = parseInt(dateNumberParsed[1], 10); // 숫자로 변환하여 앞의 0 제거
    const day = parseInt(dateNumberParsed[2], 10); // 숫자로 변환하여 앞의 0 제거
    yearSelect.value = year;
    monthSelect.value = month < 10 ? `${month}` : month; // 앞에 0 추가
    dayInput.value = day < 10 ? `${day}` : day; // 앞에 0 추가
