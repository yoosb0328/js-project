...
const names = ['진라면', '신라면', '안성탕면', '짜파게티', '불닭볶음면', '팔도비빔면', '코카콜라', '새우깡', '칙촉', '수박바'];
...
function initSales() {
    const sales = [];
    const totalSalescount = 30;

    for (let i = 0; i< totalSalescount; i++) {
        //임의의 날짜
        const randomDate = dates[Math.floor(Math.random() * dates.length)];
        //임의의 카테고리
        const randomCategory = dataStore.categories[Math.floor(Math.random() * dataStore.categories.length)];
        //읨의의 적요
